...
SC = "Sem conexão"
SBD = "                       Sem acesso ao banco de dados."
TN = "Tentar Novamente"
M = "Manutenção"
EU = "Em Uso"
...
def stock_control_notebooks(self):
    ...
    if isinstance(self.dataframe,int):
        ...
        self.table_view.setModel(self.model)
        self.delegate = ComboBoxDelegate(["Estoque", M , EU])
        ...
        self.combo_box_3.addItem(M)
        self.combo_box_3.addItem(EU)
...
def filter(self):  # PEP8: `lower_case_name` for functions
    ...
    if self.combo_box.currentText() != "Tudo":
        dataframe = dataframe.loc[dataframe.Marca == self.combo_box.currentText()] 
    if self.combo_box_2.currentText() != "Tudo":
        dataframe = dataframe.loc[dataframe.Modelo == self.combo_box_2.currentText()] 
    if self.combo_box_3.currentText() != "Tudo":
        dataframe = dataframe.loc[dataframe._Status == self.combo_box_3.currentText()]  
    if self.combo_box_4.currentText() != "Tudo":
        dataframe = dataframe.loc[dataframe.Emprestado == self.combo_box_4.currentText()]  
    self.dataframe2 = self.dataframe.copy()
...
def updated_notebook(self,column_name,column_value,id_value):
    ...
    was_maintenance = self.dataframe2.loc[self.dataframe2['Id_dispositivo'] == id_value,'_Status'].iloc[0] == M
...
def updated_smartphone(self,column_name,column_value,serial_number):
    ...
    was_maintenance = self.dataframe2.loc[self.dataframe2['Numero_serie'] == serial_number,'_Status'].iloc[0] == M
...
def borrow_notebook(self,id_dispositivo):
    ...
    if dataframe_with_id.empty:
        self.w = InvalidNotebookID()
        self.w.show()
    elif dataframe_with_id.loc[(dataframe_with_id['Emprestado']=='Não')].empty or not dataframe_with_id.loc[(dataframe_with_id['_Status']==EU)].empty:
        self.w = NotebookAlreadyBorrowed()
        self.w.show()
    elif not dataframe_with_id.loc[(dataframe_with_id['_Status']==M)].empty:
...
class AddNotebook(QtWidgets.QDialog):
    ...
    def __init__(self,parent=None):
        ...
        self.status.addItem(M)
        self.status.addItem(EU)
...
class PandasModel(QAbstractTableModel):
    ...
    def flags(self, index: QModelIndex) -> Qt.ItemFlag:
        ...
        non_editable_columns = ["Id_dispositivo", "Serial_number", "Numero_serie", "Imei_1", "Criado_por", "Modificado_por", "Emprestado", "Ultimo_comeco_manutencao", "Ultimo_final_manutencao"]  # Replace with your column names
